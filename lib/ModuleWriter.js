    ...
    writeModuleExports() {
        this.ensureFolderExists().then(() => {
            let buildOptions = {
                ...
                dependencyTree: this.module.moduleMap, 
                fileLocation: this.moduleTranslationModel.filePath,
                components: this.moduleTranslationModel.components,
                requireFix: this.moduleTranslationModel.tags.vendor,
                childModules: this.moduleTranslationModel.importDefinitions.filter(childModule => childModule.moduleDependency !== undefined).map(importedModule => importedModule.moduleDependency) || []
            };
            let buildPromise;

            if(buildOptions.requireFix)
                buildPromise = this.codeGenerator.buildExternalModuleExports(buildOptions);
            else {
                buildPromise = this.codeGenerator.buildModuleExports(buildOptions);
            }
